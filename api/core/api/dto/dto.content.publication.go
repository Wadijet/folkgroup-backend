...
type PublicationCreateInput struct {
	VideoID        string                 `json:"videoId" validate:"required" transform:"str_objectid"`      // ID của video (L7) được publish (bắt buộc, dạng string ObjectID)
	...
	Status         string                 `json:"status,omitempty" transform:"string,default=draft"`         // Trạng thái: draft, scheduled, published, archived (mặc định: draft)
...
type PublicationUpdateInput struct {
	VideoID        string                 `json:"videoId,omitempty" transform:"str_objectid,optional"` // ID của video
