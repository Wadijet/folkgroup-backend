...
**Eventbase** = `cio_events` collection — nơi ghi nhận **có chọn lọc** các sự kiện thuộc **interaction ledger**:

- **(A) Interaction events** — message, conversation, click, view, session, touchpoint
- **(B) Business events** — order_created, order_updated, payment_success (gắn với customer/conversation)
- **(C) Interaction-adjacent internal** — agent_assigned, delivery_failed, handoff_completed, …
...
### Phân Event Thành 3 Lớp

| Lớp | Mô tả | Ví dụ | Lưu trong CIO? |
|-----|-------|-------|----------------|
| **A. Interaction** | Khách chạm, hệ thống gửi, delivery result | `message_received`, `message_sent`, `touchpoint_triggered`, `order_created`, `conversation_updated` | ✅ Có |
| **B. Interaction-adjacent internal** | Internal nhưng ảnh hưởng timeline/audit tương tác | `agent_assigned`, `handoff_completed`, `delivery_failed`, `session_closed_by_timeout`, `conversation_viewed`, `note_added_to_thread` | ✅ Có |
| **C. Pure domain/internal compute** | Thuần domain khác, không phục vụ trace interaction | `customer.ltv_recomputed`, `ads.creative_scored`, `rule.batch_evaluated`, `dashboard.snapshot_built` | ❌ Không |
...
Order thuộc **Lớp A (Interaction/Business)** — gắn customer, cần audit timeline (gắn conversation với outcome).
...
| Field | Loại | Mô tả |
|-------|------|-------|
| `EventCategory` | string | `interaction` \| `business` — phân biệt (A) vs (B) |
...
### 3.5 Phase 2 — Internal Events (Lớp B — Tùy chọn)

Khi Execution Gateway, Delivery, Agent có event ảnh hưởng interaction timeline — inject vào CIO:
...
**Ref cho Lớp B:** Event vận hành nội bộ nhẹ → ref nhẹ. Chỉ cần `links.conversationUid`, `actor`, `timestamp`. Không cần `causedBy`/`resultRefs` sâu. Event từ Delivery (`delivery_failed`) → cần `causedBy.executionUid`, `resultRefs.deliveryUid`.
...
| 8 | Internal events Lớp B (delivery_failed, agent_assigned, …) | 1–2 ngày | Thấp | 2 |
...
- 2026-03-18: Bổ sung §1.1 Quy tắc lọc — chỉ lưu có chọn lọc (3 lớp A/B/C, 2 câu hỏi, eventScope, ví dụ chốt); §3.5 Internal events Lớp B; eventScope trong model và schema order
