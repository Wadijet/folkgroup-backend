...
type AIRun struct {
	...
	// Lưu lại toàn bộ quá trình chat/reasoning của AI (nếu có nhiều lượt chat)
	Messages          []AIConversationMessage  `json:"messages,omitempty" bson:"messages,omitempty"`                   // Conversation messages (system, user, assistant)
	Reasoning         string                   `json:"reasoning,omitempty" bson:"reasoning,omitempty"`                 // Reasoning/thinking process của AI (nếu có)
	IntermediateSteps []map[string]interface{} `json:"intermediateSteps,omitempty" bson:"intermediateSteps,omitempty"` // Các bước trung gian trong quá trình xử lý
...
// AIConversationMessageRole định nghĩa các role trong conversation
const (
	AIConversationRoleSystem    = "system"    // System message (instructions, context)
	AIConversationRoleUser      = "user"      // User message (prompt, input)
	AIConversationRoleAssistant = "assistant" // Assistant message (AI response)
)

// AIConversationMessage đại diện cho một message trong conversation
type AIConversationMessage struct {
	Role      string                 `json:"role" bson:"role"`                               // Role: "system", "user", "assistant"
	Content   string                 `json:"content" bson:"content"`                         // Nội dung message (TEXT)
	Timestamp int64                  `json:"timestamp,omitempty" bson:"timestamp,omitempty"` // Thời gian message (milliseconds)
	Metadata  map[string]interface{} `json:"metadata,omitempty" bson:"metadata,omitempty"`   // Metadata bổ sung (tokens, model, etc.)
}
