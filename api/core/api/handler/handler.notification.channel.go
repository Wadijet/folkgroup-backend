...
// - Business logic validation (uniqueness check phức tạp) đã được chuyển xuống NotificationChannelService.InsertOne
...
// ✅ Gọi NotificationChannelService.InsertOne (service sẽ validate uniqueness và insert)
func (h *NotificationChannelHandler) InsertOne(c fiber.Ctx) error {
	return h.SafeHandler(c, func() error {
		// Parse request body thành DTO
		var input dto.NotificationChannelCreateInput
		...
		// Business logic validation đã được chuyển xuống NotificationChannelService.InsertOne
		data, err := h.channelService.InsertOne(ctx, *model)
