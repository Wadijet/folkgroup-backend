...
type DraftPublicationCreateInput struct {
	DraftVideoID   string                 `json:"draftVideoId" validate:"required"` // ID của draft video (L7) được publish (bắt buộc, dạng string ObjectID)
	...
	Status         string                 `json:"status,omitempty"`                  // Trạng thái: draft, scheduled (mặc định: draft)
...
type DraftPublicationUpdateInput struct {
	DraftVideoID   string                 `json:"draftVideoId,omitempty"`           // ID của draft video
