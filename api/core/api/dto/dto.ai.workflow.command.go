...
// - RootRefID và RootRefType tùy chọn: bắt buộc khi step cần parent (L2-L6); không cần khi step tạo Pillar (L1)
type AIWorkflowCommandCreateInput struct {
	...
	RootRefID   string                 `json:"rootRefId,omitempty" transform:"str_objectid_ptr,optional"`         // ID của root content (dạng string ObjectID) - parent node; không cần khi tạo Pillar (L1)
	RootRefType string                 `json:"rootRefType,omitempty"`                                             // Loại root reference: "pillar", "stp", etc.; rỗng khi tạo Pillar (L1)
