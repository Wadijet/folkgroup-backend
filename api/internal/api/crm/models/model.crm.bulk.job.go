...
// CrmBulkJobType loại job bulk: sync, backfill, rebuild, recalculate_one, recalculate_all, recalculate_batch.
const (
	CrmBulkJobSync             = "sync"
	...
	CrmBulkJobRecalculateBatch = "recalculate_batch" // Job batch: params { offset, limit } — dùng cho recalculate-all chunking
)
...
type CrmBulkJob struct {
	...
	Params              bson.M             `json:"params" bson:"params"` // sources, types, limit, unifiedId, offset...
	...
	Progress            bson.M             `json:"progress,omitempty" bson:"progress,omitempty"` // Tiến độ để resume khi restart: offset, skip, counts...
}
