...
**Mục đích:** Tài liệu kiến trúc chính thức (canonical) cho module Rule Intelligence — đơn vị biến đổi cốt lõi của pipeline trí tuệ, nằm trước Decision Brain.

**Liên quan:** [Decision Brain](decision-brain.md), [Activity Framework](activity-framework.md), [Ads Metrics Pipeline](ads-metrics-pipeline.md)
...
- Traceability đầy đủ cho Decision Brain học
...
**Mục tiêu tách Logic vs Param:** Cho phép đánh giá từng rule theo 2 lớp — (1) Logic có đúng spec? (2) Param có tối ưu? Decision Brain / A/B test có thể phân tích riêng từng lớp.
...
| Pattern | Vị trí hiện tại | Mô tả | Chuyển thành Rule |
|---------|-----------------|-------|-------------------|
| **Metric calculations** | `service.meta.evaluation` — `computeLayer1`, `computeLayer2`, `computeLayer3` | CTR, ConvRate, ROAS, CHS, MQS, lifecycle, portfolioCell | Derivation Rule: raw → layer1 → layer2 → layer3 |
| **Threshold detection** | `ads/rules/flag_evaluator` — `EvaluateCondition`, `EvaluateFlags` | So sánh fact với ngưỡng (>, <, >=, <=, IN) | Interpretation Rule: metric → flag |
| **Anomaly detection** | `DetectWindowShoppingPattern`, CHS exception | Pattern bất thường (mess cao, orders thấp; CHS đột ngột critical) | Interpretation Rule + Governance Rule |
| **Scoring models** | `scoreFromRoas`, `scoreFromRate`, `scoreFromCpmCtr`, `scoreFromFrequency` | Chuyển metric → điểm 0–100 | Derivation Rule |
| **Classification rules** | `derivePortfolioCell`, `healthState` | Lifecycle × performanceTier → cell; CHS → healthState | Derivation Rule |
| **Automation triggers** | `computeSuggestedActions`, `EvaluateAlertFlagsWithConfig`, `EvaluateForDecreaseWithConfig` | Flag → action (PAUSE, DECREASE, INCREASE) | Execution Rule |
| **Governance conditions** | `isLifecycleNew`, `ShouldAutoPropose`, noon cut, dual-source confirm | Điều kiện an toàn trước khi đề xuất | Governance Rule |
...
| `trace_id` | Để audit, debug, liên kết với Decision Brain. |
...
Hệ thống ghi **full rule execution trace** cho mỗi lần chạy — phục vụ debugging, audit, observability, Decision Brain learning.
...
| **Learning** | Trace + Outcome (từ Decision Brain) → phân tích rule có hiệu quả không |
...
## 12. Tích Hợp Với Decision Brain
...
| **Decision Brain** | Phân tích outcome sau khi entity đóng | Sau khi lifecycle closed |
...
### 12.3 Decision Brain Đề Xuất Cải Thiện Rule

- Decision Brain phân tích: rule X trigger → outcome success/partial/failed
...
**Lợi ích:** Tách bạch logic vs param → Decision Brain / A/B test có thể đánh giá: "Rule sl_a logic đúng nhưng param chưa tối ưu" hoặc "Rule sl_b logic cần sửa".
