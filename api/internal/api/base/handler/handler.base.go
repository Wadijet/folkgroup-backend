...
// Package basehdl chứa base handler (BaseHandler, SafeHandlerWrapper, CRUD, response, system).
...
// GetActiveOrganizationID lấy active organization ID từ context (export để domain handler gọi).
...
// SetOrganizationID gán ownerOrganizationId vào model (export để domain handler gọi).
...
// GetOwnerOrganizationIDFromModel lấy ownerOrganizationId từ model (export để domain handler gọi).
...
// Export để auth/handler (package khác) có thể gọi.
func (h *BaseHandler[T, CreateInput, UpdateInput]) ValidateUserHasAccessToOrg(c fiber.Ctx, orgID primitive.ObjectID) error {
	...
	allowedOrgIDs, err := authsvc.GetAllowedOrganizationIDsFromRole(c.Context(), activeRoleID, permissionName)
	if err != nil {
		return err
	}
	...
	sharedOrgIDs, err := authsvc.GetSharedOrganizationIDs(c.Context(), allowedOrgIDs, permissionName)
	if err == nil {
...
// ValidateOrganizationAccess validate user có quyền truy cập document này không (export để domain handler gọi).
...
type BaseHandler[T any, CreateInput any, UpdateInput any] struct {
	BaseService   basesvc.BaseServiceMongo[T] // Service xử lý logic nghiệp vụ với MongoDB
	filterOptions FilterOptions                // Cấu hình validate filter
}

// NewBaseHandler tạo mới một BaseHandler với BaseService được cung cấp
func NewBaseHandler[T any, CreateInput any, UpdateInput any](baseService basesvc.BaseServiceMongo[T]) *BaseHandler[T, CreateInput, UpdateInput] {
...
// SetFilterOptions gán FilterOptions cho BaseHandler (dùng bởi domain handler trong package khác, ví dụ ctahdl).
func (h *BaseHandler[T, CreateInput, UpdateInput]) SetFilterOptions(opts FilterOptions) {
	h.filterOptions = opts
}

// ValidateInput thực hiện validate chi tiết dữ liệu đầu vào (export để domain handler gọi).
...
// ProcessFilter xử lý filter từ query (export để domain handler gọi).
...
// NormalizeFilterValue chuẩn hóa giá trị filter (export để domain handler gọi).
...
// getCreateInputBSONKeySet trả về tập tên key BSON (trong document) tương ứng với các field của CreateInput.
// Dùng khi Upsert: chỉ đưa vào $set những field có trong input (kể cả giá trị 0/false), không ghi đè field không gửi.
// Phân biệt: "có field, value = 0" (ghi vào DB) vs "không có field trong input" (không ghi).
//
// Returns:
// - map[string]bool: key = tên BSON trong document, value = true. Nil nếu không xác định được (fallback logic cũ).
func (h *BaseHandler[T, CreateInput, UpdateInput]) getCreateInputBSONKeySet() map[string]bool {
	...
	if inputType.Kind() != reflect.Struct || modelType.Kind() != reflect.Struct {
		return nil
	}
	keySet := make(map[string]bool)
	for i := 0; i < inputType.NumField(); i++ {
		inputField := inputType.Field(i)
		if !inputField.IsExported() {
			continue
		}
		targetFieldName := inputField.Name
		if transformTag := inputField.Tag.Get("transform"); transformTag != "" {
			config, err := utility.ParseTransformTag(transformTag)
			if err != nil {
				continue
			}
			if config.MapTo != "" {
				targetFieldName = config.MapTo
			}
		}
		...
		bsonTag := modelField.Tag.Get("bson")
		if bsonTag == "" || bsonTag == "-" {
			continue
		}
...
// TransformCreateInputToModel chuyển CreateInput sang Model (export để domain handler gọi).
...
//   - *T: Model đã được transform
//   - error: Lỗi nếu có trong quá trình transform
// TransformUpdateInputToModel chuyển UpdateInput sang Model (export để domain handler gọi).
