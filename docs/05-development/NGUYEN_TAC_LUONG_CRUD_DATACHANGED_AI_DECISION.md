...
**Field `pipelineStage` trên `decision_events_queue`:** bổ sung cho `eventType` / `eventSource` — mô tả **bước trong khung quy trình tổng** (sau persist nguồn, sau merge L2, sau worker intel, điều phối nội bộ AID, ingest HTTP/API). Hằng số một nguồn: `api/internal/api/aidecision/eventtypes/pipeline_stage.go`. Bảng giá trị và quan hệ với luồng: [co-cau-module-aid-va-domain-queue.md](../../docs/module-map/co-cau-module-aid-va-domain-queue.md) mục 11.
