...
type AuthManager struct {
	...
	RolePermissionCRUD *authsvc.RolePermissionService
	UserRoleCRUD       *authsvc.UserRoleService
	Cache              *utility.Cache
}
...
func newAuthManager() (*AuthManager, error) {
	...
	// Khởi tạo các service auth (từ domain auth)
...
func (am *AuthManager) getUserPermissions(userID string, activeRoleID *primitive.ObjectID) (map[string]byte, error) {
	...
	if activeRoleID != nil {
		...
		_, err := am.UserRoleCRUD.BaseServiceMongoImpl.FindOne(context.TODO(), bson.M{
		...
		findRolePermissions, err := am.RolePermissionCRUD.BaseServiceMongoImpl.Find(context.TODO(), bson.M{"roleId": *activeRoleID}, nil)
		if err != nil {
			am.Cache.Set(cacheKey, permissions)
			return permissions, nil
		}
		...
		for _, rolePermission := range findRolePermissions {
			permission, err := am.PermissionCRUD.BaseServiceMongoImpl.FindOneById(context.TODO(), rolePermission.PermissionID)
			if err != nil {
				continue
			}
		...
		findRoles, err := am.UserRoleCRUD.BaseServiceMongoImpl.Find(context.TODO(), bson.M{"userId": utility.String2ObjectID(userID)}, nil)
		if err != nil {
		...
		for _, userRole := range findRoles {
			...
			findRolePermissions, err := am.RolePermissionCRUD.BaseServiceMongoImpl.Find(context.TODO(), bson.M{"roleId": userRole.RoleID}, nil)
			if err != nil {
				continue
			}
...
func AuthMiddleware(requirePermission string) fiber.Handler {
	...
	return func(c fiber.Ctx) error {
		...
		user, err = authManager.UserCRUD.BaseServiceMongoImpl.FindOne(context.Background(), query, nil)
