...
type Renderer struct {
	ctaLibraryService *ctasvc.CTALibraryService
}
...
func (r *Renderer) FindCTA(ctx context.Context, code string, organizationID primitive.ObjectID) (*ctamodels.CTALibrary, error) {
...
func (r *Renderer) RenderCTA(cta *ctamodels.CTALibrary, variables map[string]interface{}) (string, string, error) {
	// Render label
	label := cta.Label
...
func TrackCTAClick(ctx context.Context, historyID primitive.ObjectID, ctaIndex int, ctaCode string, ownerOrganizationID primitive.ObjectID, ipAddress, userAgent string) error {
	trackingService, err := ctasvc.NewCTATrackingService()
	if err != nil {
		return fmt.Errorf("failed to create CTA tracking service: %w", err)
	}

	tracking := ctamodels.CTATracking{
