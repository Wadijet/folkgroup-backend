...
| **2 — Vị trí trong quy trình chuẩn** | `e2eStage`, `e2eStepId`, `e2eStepLabelVi` (= **`descriptionUserVi`** catalog §5.3 cho `e2eStepId`, một nguồn với JSON `steps`) + dòng đầu `DetailBullets` **«Trong quy trình: Gx-Syy — …»** (sau `enrichPublishE2ERef`) | «Mốc này tương ứng **bước nào** trong bảng G1–G6?» — đối chiếu doc và `e2e_reference.go` / `e2e_catalog.go`.                 |
...
**Quy ước trùng lặp có chủ đích:** `summary` và tiêu đề một dòng (`step.title` → `uiTitle`) **không** mang tiền tố kiểu `G1 —` / `G2 —` vì vị trí Gx-Syy đã có ở lớp 2; `phase` / `phaseLabelVi` vẫn phục vụ **lọc** và badge. Trong `decisionlive.resolveE2ERefForPublish`: **ưu tiên** `ResolveE2EForLivePhase(phase)` khi `phase` map được đủ `e2eStage` + `e2eStepId` catalog (khớp §5.3 / `E2ELivePhaseCatalog`); **sau đó** mới envelope `Refs` (`eventType` / `eventSource` / `pipelineStage`) qua `ResolveE2EForQueueEnvelope` — để mốc timeline neo đúng hàng **phase** trong bảng chi tiết, không bị lệch về bước enqueue G1 khi vẫn còn `eventType` job queue trên `Refs`.
...
**Ghi chú hai cột module (§5.2):** **Module giao việc** = lớp phát lệnh enqueue / handoff (tinh thần `enqueueSourceDomain` — `aidecision/crmqueue/domain_queue_bus.go`, [§1.4](#14-meta-trên-document-job-hàng-đợi-miền-đồng-bộ-bus--chuẩn-gộp-một-nguồn)). **Module thực hiện** = worker/package chạy bước chủ đạo (tinh thần `processorDomain` / `businessDomain` — [§1.2](#12-miền-nghiệp-vụ-bounded-context--tên-gọi-thống-nhất)). **Nhóm trách nhiệm** ở §5.3 lấy từ code (`responsibilityGroup`: `DomainData` vs `DomainIntel` — **không** ghi chung là `Domain`). Một pha có **nhiều** vai trò — bảng §5.2 ghi **điển hình**; chi tiết từng bước xem §5.3. JSON `e2e-reference-catalog` · `stages` **chưa** có hai cột module.
...
**Phân biệt «Nhóm trách nhiệm» và «Module thực hiện» (hai cột dễ thấy trùng ý):** **Nhóm trách nhiệm** khớp **đúng chữ** trường `responsibilityGroup` trong JSON `steps` (sinh từ `E2EStepCatalog()` trong `eventtypes/e2e_catalog.go`) — tập giá trị cố định: `CIO`, `AID`, **`DomainData`** (merge L1→L2 / báo sau L2), **`DomainIntel`** (job `*_intel_compute`, emit `*_intel_recomputed`), `Executor`, `Outcome`, `Learning`, `Feedback`. **Không** có giá trị rút gọn `Domain` trong API. Đối chiếu swimlane §1.1: `DOM` ↔ `DomainData`, `INT` ↔ `DomainIntel`. **Module thực hiện** / **Module giao việc** là **mở rộng doc** (không có trong `steps`): gợi ý package/worker và tinh thần `processorDomain` / `enqueueSourceDomain` trên document job miền — [§1.2](#12-miền-nghiệp-vụ-bounded-context--tên-gọi-thống-nhất), [§1.4](#14-meta-trên-document-job-hàng-đợi-miền-đồng-bộ-bus--chuẩn-gộp-một-nguồn), hằng `ProcessorDomain*` / `EnqueueSource*` trong `aidecision/crmqueue/domain_queue_bus.go`. Một ô có thể ghi **nhiều** actor. UI một trục: ưu tiên **Nhóm trách nhiệm**; debug Mongo/repo: ưu tiên hai cột module.

**Hai cột mô tả (v10):** `**descriptionTechnicalVi`** và `**descriptionUserVi`** — JSON/API và `E2EStepCatalog()` trong `eventtypes/e2e_catalog.go` giữ **bản đầy đủ** (audit, đồng bộ từng chữ). **Bảng Markdown §5.3 dưới đây** dùng **bản rút gọn** cột kỹ thuật để đọc nhanh; tra chi tiết wire, resolver, legacy → các cột `eventType` / `eventSource` / `pipelineStage` và mục G1–G4 phía trên. Ghi nhớ nhanh: **G1-S04** = enqueue báo L1; **G2-S05-E01** = báo sau L2; **G3-S06** = emit `*_intel_recomputed` (AID đọc job → resolver **G4-S01**); **G4-S02** = một dòng context requested/ready; **`message.batch_ready`** → **G2-S02** (v34+); execute/propose → **G4-S03-E01…E03** (v35).
...
**Hai cột module (bổ sung §5.3):** **Module giao việc (nếu có)** và **Module thực hiện** chỉ có trên Markdown; **không** trùng với `responsibilityGroup` trong JSON. Ghi theo luồng code: consumer / emit (`worker.aidecision.consumer`, `aidecision/crmqueue`, worker `*_intel_compute`, …) và meta job miền (`processorDomain` = `crm` \| `order` \| `cix` \| `ads`, `enqueueSourceDomain` = `aidecision` \| `crm_datachanged` \| … — `domain_queue_bus.go`). JSON `e2e-reference-catalog` · `steps` **chưa** có hai cột này tới khi nâng schema (`responsibilityGroup` đã có sẵn, khớp cột **Nhóm trách nhiệm**).
...
| Giai đoạn | Bước | Sự kiện | Mô tả kỹ thuật (`descriptionTechnicalVi`) — **rút gọn**; bản đầy đủ trong `e2e_catalog.go` / JSON | Mô tả người dùng (`descriptionUserVi`) | `eventType` | `eventSource` | `pipelineStage` | Module giao việc (nếu có) | Module thực hiện | Nhóm trách nhiệm |
| --------- | ------ | ---------- | --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------ | ------------------------- | ---------------- | ----------------
| G1 | G1-S01 | — | Ingress nhận payload kênh ngoài; xác thực — chưa L1. | Cửa hàng nhận tin từ khách qua các kênh (webhook, đồng bộ…); mới kiểm tra và ghi nhận, chưa lưu vào kho dữ liệu nội bộ. | — | — | — | **—** | **Ingress / CIO** | `CIO` |
| G1 | G1-S02 | — | Chuẩn hoá payload + ghi L1 (`DoSyncUpsert` / CRUD domain). | Dữ liệu nghiệp vụ được chuẩn hoá và lưu bản ghi nguồn (L1) cho các bước sau. | — | — | — | **—** | **Ingress / CIO** (ghi L1) | `CIO` |
| G1 | G1-S03 | — | `EmitDataChanged`; `hooks/datachanged` lọc collection (`ShouldEmitDatachangedToDecisionQueue`) → chỉ hợp lệ mới tới G1-S04. | Sau khi lưu xong, hệ thống báo nội bộ có thay đổi; chỉ những loại dữ liệu đã cấu hình mới được chuyển tiếp tới hàng đợi trợ lý. | — | — | — | **aidecision** (lọc `hooks/datachanged`) | **CIO** + **aidecision** | `CIO` |
| G1 | G1-S04 | — | Enqueue `decision_events_queue` báo đổi L1: `<prefix>.changed`, `l1_datachanged` / `after_l1_change`; payload `sourceCollection`, `dataChangeOperation`; legacy `*.inserted|updated` qua `consumerreg`. | Mỗi thay đổi nguồn đã đăng ký được đưa vào hàng đợi trợ lý; loại bản ghi biết qua collection. | `<prefix>.changed` — prefix từ collection | `l1_datachanged` | `after_l1_change` | **aidecision** (enqueue `decision_events_queue`) | **aidecision** | `CIO` |
| G2 | G2-S01 | — | Consumer lease job `decision_events_queue` → bắt đầu `processEvent`; điển hình `l1_datachanged` — mọi loại job đều qua bước lease (timeline G2-S01). | Máy lấy một việc đã xếp hàng; trên luồng merge thường là một sự kiện báo đổi L1 (`l1_datachanged`). | — | — | — | **—** | **aidecision** (consumer — lease job) | `AID` |
| G2 | G2-S02 | — | `processEvent`: datachanged — gom (debounce) / gấp (urgency); `applyDatachangedSideEffects` → routing / `dispatchConsumerEvent` / `consumerreg`; mốc queue → G2-S02; `message.batch_ready` → resolver G2-S02 (v34). | Có thể gom nhiều cập nhật theo cửa sổ (debounce) để đỡ tải; khi gấp, bỏ phần gom và ưu tiên trước nhưng vẫn qua đủ đồng bộ — có thể gửi yêu cầu gộp sang worker miền; khi miền xong, việc quay lại hàng đợi trợ lý để chạy tiếp; gom tin nhắn batch neo **G2-S02** trên resolver. | — | — | — | **aidecision** (routing / side-effect) | **aidecision** (`processEvent`) | `AID` |
| G2 | G2-S03 | — | Worker miền lấy job merge L1→L2 (vd. `crm_pending_merge` / `CrmPendingMergeWorker`) — ngoài consumer AID. | Worker miền nhận việc gộp đã xếp từ bước trước (vd. hàng đợi merge CRM) và thực hiện gộp dữ liệu. | — | — | — | **aidecision** (đã xếp job merge) | **worker miền** (vd. **`crm`** — `crm_pending_merge` / `CrmPendingMergeWorker`) | `DomainData` |
| G2 | G2-S04 | — | Ghi canonical L2 (uid, sourceIds, links). | Cập nhật hồ sơ chung để mọi kênh trỏ cùng một thực thể. | — | — | — | **—** | **worker miền** (merge L1→L2) | `DomainData` |
| G2 | G2-S05 | G2-S05-E01 | Sau merge: enqueue bus báo L2 — `l2_datachanged` + `<prefix>.changed` + `after_l2_merge` (vd. `EmitAfterL2MergeForCrmIntel`); consumer `IsPostL2MergeCrmIntelEnvelope` → intel CRM; legacy `crm_merge_queue` / `recompute_requested` vẫn G2-S05-E01. | Miền gộp xong hồ sơ chung (L2) rồi báo thay đổi lên hàng đợi AID như bước báo đổi L1 — để trợ lý xử lý tiếp. | `<prefix>.changed` — prefix từ collection | `l2_datachanged` | `after_l2_merge` | **miền** (`crmqueue.EmitAfterL2MergeForCrmIntel` / `notify_after_crm_merge`) | **miền** (**crm** — sau merge, emit bus AID) | `DomainData` |
| G3 | G3-S01 | — | Consumer AID xử lý job đầu vào intel (gom nhiều prefix `crm|ads|order|cix`); enqueue `*_intel_compute` ở G3-S02; `*_intel_recomputed` khi AID đọc → resolver G4-S01. (`.changed`+L2 có thể map G2-S05-E01, không phải G3-S01.) | Trợ lý đang xử lý một việc trên hàng đợi liên quan làm mới phân tích — thường là báo đổi L2 sau khi hồ sơ chung cập nhật; cùng một bước catalog cho các tên yêu cầu intel khác trên wire. | `<domain>.*` — crm / ads / order / cix (vd. `crm.intelligence.*_requested`); điển hình sau L2: `<prefix>.changed` + `l2_datachanged` | điển hình `l2_datachanged`; tuỳ miền và emit (crm, meta_*, aidecision, …) | điển hình `after_l2_merge`; tuỳ emit (`after_l1_change`, `external_ingest`, …) | **bus / miền** (enqueue đầu vào intel; xem §1.4) | **aidecision** (consumer — envelope intel) | `AID` |
| G3 | G3-S02 | — | AID: routing + gom/gấp → enqueue `*_intel_compute` từng miền (vd. nhánh `IsPostL2MergeCrmIntelEnvelope`); không chạy intel nặng tại đây. | AID quyết định và tạo việc tính lại intelligence trên hàng đợi từng domain (có gom/gấp). | — | — | — | **aidecision** (xếp `*_intel_compute`) | **aidecision** | `AID` |
| G3 | G3-S03 | — | Worker miền lease job `*_intel_compute`. | Worker của từng domain lấy job tính toán lại intelligence | — | — | — | **aidecision** (job đã vào `*_intel_compute`) | **worker miền** (`crm_intel_compute` / `order_intel_compute` / `cix_intel_compute` / `ads_intel_compute` — `ProcessorDomain*` trong `domain_queue_bus.go`) | `DomainIntel` |
| G3 | G3-S04 | — | Pipeline intel (rule / LLM / snapshot). | Worker của từng domain chạy tính toán lại intelligence | — | — | — | **—** | **worker miền** (pipeline intel) | `DomainIntel` |
| G3 | G3-S05 | — | Persist intel + read model; live `intel_domain_compute_done` → G3-S05. | Lưu kết quả tính toán lại intelligence | — | — | — | **—** | **worker miền** (read model) | `DomainIntel` |
| G3 | G3-S06 | — | Miền emit `<domain>_intel_recomputed` lên queue (catalog = bước emit); AID consumer đọc cùng envelope → resolver **G4-S01**. | Miền bắn event đã tính toán xong intelligence lên hàng đợi AID để tiếp tục xử lý | `<domain>_intel_recomputed` | `cix_intel`, `crm_intel`, `order_intel`, `meta_ads_intel` | `domain_intel` | **miền** (emit handoff — `intelrecomputed` / emit service từng miền) | **worker miền** (hoàn tất intel trước khi emit) | `DomainIntel` |
| G4 | G4-S01 | — | `ResolveOrCreate` / cập nhật `decision_cases_runtime`; điển hình job `*_intel_recomputed` → `ResolveE2EForQueueEnvelope` G4-S01. | Trợ lý mở hoặc cập nhật vụ việc — thường khi nhận bàn giao phân tích từ miền (`*_intel_recomputed`) hoặc các luồng case khác. | — | — | — | **miền** (`*_intel_recomputed`) | **aidecision** (case runtime) | `AID` |
| G4 | G4-S02 | — | Một dòng catalog: `*.context_requested` / `*.context_ready` — resolver G4-S02 (không tách `eventDetailId`). | Trợ lý xin thêm hoặc nhận báo đủ dữ liệu nền — phân biệt requested / ready qua `eventType`. | `<domain>.context_requested` / `<domain>.context_ready` — vd. customer., ads. | `aidecision`; `crm`; `meta_ads_intel` (tuỳ event) | `aid_coordination` | **aidecision** (`*.context_requested`) / **miền** (`*.context_ready`) | **aidecision** + **miền** | `AID` |
| G4 | G4-S03 | — | Đủ context → policy + engine → `ResolveE2EForLivePhase` G4-S03; phát execute/propose (wire E01–E03). | Trợ lý xác nhận đủ thông tin, áp quy tắc, suy luận và chuẩn bị hoặc gửi hành động phù hợp. | — | — | — | **—** | **aidecision** (policy + engine) | `AID` |
...
Chi tiết từng bước `Gx-Syy`: API JSON `steps` và `eventtypes/e2e_catalog.go` (mô tả kỹ thuật đầy đủ); bảng §5.3 phía trên là bản đọc nhanh.
...
- **2026-04-19:** Code `livecopy/domain_event.go` — `DomainNarrativeFromQueueEvent`: **khung** `StepTitle` / `BusinessOneLine` chỉ từ **`descriptionUserVi`** catalog (§5.3) theo `ResolveE2EForQueueEnvelope`; `switch` chỉ thêm **EntityBullets** tình huống (đơn, hội thoại, chiến dịch, hồ sơ, nguồn…); bỏ văn tự viết riêng từng `eventType`.
- **2026-04-19:** Code `decisionlive/persist_org_audit.go` — `phaseLabelVi` bổ sung **`intel_domain_compute_*`** và **`ads_evaluate`**: không còn trả nguyên chuỗi `phase` tiếng Anh trên UI lưu đồ miền; start/done/ads dùng `descriptionUserVi` catalog (G3-S03, G3-S05, G4-S03); lỗi worker miền dùng nhãn lỗi tiếng Việt cố định. Test `TestPhaseLabelVi_*`.
- **2026-04-19:** Code `eventtypes` — `E2ERef.LabelVi` (và payload `e2eStepLabelVi`) lấy từ **`descriptionUserVi`** trong `E2EStepCatalog()` (`E2ECatalogDescriptionUserViForStep`) cho mọi nhánh `ResolveE2EForQueueEnvelope` / `ResolveE2EForLivePhase` và mốc consumer (fallback `userLabelVi` milestone); test `E2ECatalogDescriptionUserViForStep_ResolverStepIDsDayDu`.
- **2026-04-19:** Code `decisionlive/publish.go` — `resolveE2ERefForPublish` **ưu tiên** `ResolveE2EForLivePhase(phase)` khi `phase` map đủ `e2eStage` + `e2eStepId` (khớp §5.3 / `E2ELivePhaseCatalog`), rồi mới envelope `Refs` — timeline Publish neo đúng bảng chi tiết theo mốc phase (vd. `orchestrate` → **G4-S01**, `consuming` → **G4-S03** thay vì lệch theo `eventType` job queue). Doc §4.2 một đoạn quy ước; test `publish_resolve_e2e_test.go`.
- **2026-04-19:** Doc §5.3 — cột **Nhóm trách nhiệm** đối chiếu `ResponsibilityGroup` trong `eventtypes/e2e_catalog.go`: **G2-S03…G2-S05-E01** = `DomainData`, **G3-S03…G3-S06** = `DomainIntel` (thay nhãn markdown cũ `Domain`); làm rõ hai cột **module** (`domain_queue_bus.go`); cập nhật mục phân biệt + ghi chú §5.2. **Cột mô tả kỹ thuật** bảng catalog Markdown **rút gọn** — bản đầy đủ trong `e2e_catalog.go` và JSON `steps`.
