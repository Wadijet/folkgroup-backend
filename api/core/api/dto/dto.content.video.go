...
type VideoCreateInput struct {
	ScriptID     string                 `json:"scriptId" validate:"required" transform:"str_objectid"` // ID của script (L6) tạo ra video này (bắt buộc, dạng string ObjectID)
	...
	Status       string                 `json:"status,omitempty" transform:"string,default=pending"`  // Trạng thái: pending, rendering, ready, failed, archived (mặc định: pending)
...
type VideoUpdateInput struct {
	ScriptID     string                 `json:"scriptId,omitempty" transform:"str_objectid,optional"` // ID của script
