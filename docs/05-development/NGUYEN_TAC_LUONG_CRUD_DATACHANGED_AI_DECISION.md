...
- **Map `DatachangedEmitPerCollection`** (`datachanged_emit_per_collection.go`): key có trong map → dùng `true`/`false`; collection **không** có key: non-Meta → emit; nhóm Meta Marketing → **chỉ `meta_ad_insights`** emit (cố định trong code).
